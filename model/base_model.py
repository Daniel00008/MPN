...
class Decoder_new(torch.nn.Module):
    ...
    def forward(self, x, skip1, skip2, skip3):
        tensorConv = self.moduleConv(x) # 512

        tensorUpsample4 = self.moduleUpsample4(tensorConv) # 256
        cat4 = torch.cat((skip3, tensorUpsample4), dim=1) # 512

        tensorDeconv3 = self.moduleDeconv3(cat4) # 256
        tensorUpsample3 = self.moduleUpsample3(tensorDeconv3) # 128
        cat3 = torch.cat((skip2, tensorUpsample3), dim=1) # 256

        tensorDeconv2 = self.moduleDeconv2(cat3) # 128
        tensorUpsample2 = self.moduleUpsample2(tensorDeconv2) #64
        cat2 = torch.cat((skip1, tensorUpsample2), dim=1) # 128

        return cat2


class convAE(torch.nn.Module):
    def __init__(self, n_channel=3, t_length=5, proto_size=10, feature_dim=512, key_dim=512, temp_update=0.1,
                 temp_gather=0.1):
    ...
    def forward(self, x, weights=None, train=True):
        """
        :param x: torch.Size([8, 12, 256, 256])
        :param weights:
        :param train:
        :return:
        """
        ...
        new_fea = F.normalize(new_fea, dim=1) # torch.Size([8, 128, 256, 256])
        if train:
            updated_fea, keys, fea_loss, cst_loss, dis_loss = self.prototype(new_fea, new_fea, weights, train) # weight = None
            if weights == None:
