...
class Meta_Prototype(nn.Module):
    ...
    def get_score(self, pro, query):
        ...
        bs, m, d = pro.size()
        # import pdb;pdb.set_trace()
        score = torch.bmm(query, pro.permute(0, 2, 1))  # b , h X w , m torch.Size([8, 65536, 10])
    ...
    def forward(self, key, query, weights, train=True):
        ...
        if train:
            if weights == None:
                ...
                # key : torch.Size([8, 256, 256, 256])multi_heads_weights: torch.Size([8, 256, 256, 10])
            else:
            ...
            # Retrieving
            updated_query, fea_loss, cst_loss, dis_loss = self.query_loss(query, protos, weights,
                                                                          train)  # torch.Size([8, 65536, 128]) torch.Size([]) torch.Size([1]) torch.Size([])

            # skip connection aggregated
        ...
        # test
            ...
            key = key.reshape((batch_size, w * h, dims))
            protos = multi_heads_weights * key.unsqueeze(-2)  # torch.Size([8, 65536, 10, 128])
            protos = protos.sum(1)  # torch.Size([8, 10, 128])
    ...
    def query_loss(self, query, keys, weights, train):
        ...
        if train:
            ...
            # Diversity loss
            keys_ = F.normalize(keys, dim=-1)  # torch.Size([8, 10, 128])
            dis = 1 - distance(keys_.unsqueeze(1), keys_.unsqueeze(2))  # torch.Size([8, 10, 10])

            mask = dis > 0
            dis *= mask.float()
            dis = torch.triu(dis, diagonal=1)  # 返回矩阵上三角部分，其余部分定义为0
            dis_loss = dis.sum(1).sum(1) * 2 / (
                    self.proto_size * (self.proto_size - 1))  # loss= sum()/self.proto_size/(self.proto_size-1)
            ...
            new_query = new_query.sum(2)  # torch.Size([8, 65536, 128])
            new_query = F.normalize(new_query, dim=-1)

            # Compactness loss
            # maintain the distinction among attribute vectors
            _, gathering_indices = torch.topk(softmax_score_proto, 2, dim=-1)  # torch.Size([8, 65536, 2])

            # 1st closest memories
            pos = torch.gather(keys, 1,
                               gathering_indices[:, :, :1].repeat((1, 1, dims)))  # torch.gather 作用：收集输入的特定维度指定位置的数值
